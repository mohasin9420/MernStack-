...
const books = [
    ...
    { title: 'Book Six', genre: 'Fiction', publish: 1999, edition: 2010 },
    { title: 'Book Seven', genre: 'History', publish: 1999, edition: 1996 },
    { title: 'Book Eight', genre: 'Science', publish: 1999, edition: 2016 },
    { title: 'Book Nine', genre: 'Non-Fiction', publish: 1999, edition: 1989 },
  ];
  ...
  const userprice=books.filter( (prime) => prime.publish>2000);
  // console.log(userprice);

  const mac = books
  .map((book) => book.publish + 1)
  .filter((year) => year > 2000);

// console.log(mac);
const mac1 = books
  .map((book) => ({ ...book, publish: book.publish + 1 }))
